...
use crate::{protos::module::{ComputerStatsData, CpuLoadData, CpuData, NetworkData, MemoryData}, channel::CHANNEL_VALUE};
...
impl ModuleClient for SystemStatsModule {
	fn run(&self, _receiver_config: std::sync::mpsc::Receiver<crate::channel::ModuleConfig>) -> tokio::task::JoinHandle<Result<(), ()>> {
		return tokio::spawn(async move {
			...
			loop {
				...
				if let Ok(netifs) = sys.networks() {
					data.set_netifs(netifs.values().filter(|x| x.name.starts_with("w") || x.name.starts_with("e")).map(|x| {
				...
				CHANNEL_VALUE.0.lock().unwrap().send(("CSS".to_string(), data.write_to_bytes().unwrap())).unwrap();
