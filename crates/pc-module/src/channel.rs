use std::{sync::{Mutex, mpsc::{Sender, Receiver, channel}, Arc}, collections::HashMap};
...
impl ModuleConfigChannelManager {
	...
	pub fn new() -> Self {
		return ModuleConfigChannelManager{ senders: Arc::new(Mutex::new(ModuleSenderMap::new()))};
	}
	...
	pub fn get_reference(&self) -> ModuleSenderMapReference {
		return ModuleSenderMapReference { map: self.senders.clone() };
	}
}