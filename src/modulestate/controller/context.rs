...
use crate::{
    ...
    protos::{
        alarm::FieldAlarmEvent, env_controller::EnvironmentControllerConfiguration, module::Actor,
    },
    socket::ss::SenderSocket,
};
...
macro_rules! send_event {
    ($self: ident, $state: expr, $running: ident) => {
        ...
        if let Err(err) = $self
            .sender_socket
            .send(format!("/env_ctr/{}", $self.config.id), Box::new(event))
        {
            log::error!("{:?}", err);
        }
    };
...
macro_rules! get_env_element {
    ($self: ident, $element: ident, $value: ident) => {
        $self.config.$element.iter().find(|x| x.name.eq($value))
    };
}
