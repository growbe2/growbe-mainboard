...
pub struct EnvControllerStore {
    ...
    pub value_senders: HashMap<String, Sender<ModuleValueChange<f32>>>,
}
...
impl EnvControllerStore {
    ...
    pub fn on_alarm_created(
        ...
        self.alarm_senders.insert(key, sender);

        log::debug!(
            "creating field alarm event channel for {}:{}",
            module_id,
            property
        );
    ...
    pub fn on_alarm_deleted(
        ...
        log::debug!(
            "deleting field alarm event channel for {}:{}",
            module_id,
            property
        );
    ...
    pub fn on_module_connected(
        ...
        self.value_senders.insert(module_id.into(), sender);
    ...
    pub fn register_controller(
        ...
        log::debug!(
            "environment_controller {} has been register to the database",
            id
        );
        ...
        log::debug!(
            "environment_controller {} cannot be started for now, missing ressources",
            id
        );
    ...
    pub fn unregister_controller(&mut self, id: &str) -> Result<(), MainboardError> {
        if let Some(value) = self.tasks.remove(id) {
            log::debug!(
                "environment_controller {} still alive {}",
                id,
                value.handler.is_finished()
            );
            value.cancellation_token.cancel();
            value.cancellation_token.cancel();
            value.cancellation_token.cancel();
            log::debug!(
                "environment_controller {} task has been cancelled {}",
                id,
                value.handler.is_finished()
            );
        }
        ...
        let res =
            crate::store::database::store_delete_key(&self.conn, "environment_controller", id);
        if res.is_ok() {
            log::debug!(
                "environment_controller {} has been unregister to the database",
                id
            );
        }
        return res;
    ...
    fn create_task(
        ...
        log::debug!(
            "environment_controller task has been started {} running {}",
            config.get_id(),
            !entry.handler.is_finished()
        );
    ...
    fn create_context(
        ...
        for obs in config.observers.iter() {
            ...
            if let Some(sr) = self.alarm_senders.get(&key) {
                ...
                return Err(MainboardError::from_error(format!(
                    "failed to get field alarm event receive"
                )));
            }
            ...
            if let Some(sr) = self.value_senders.get(obs.get_id()) {
                ...
                return Err(MainboardError::from_error(format!(
                    "failed to get field value event receive"
                )));
            }
        }
...
mod tests {
    ...
    fn init(
        ...
        let conn_database = Arc::new(Mutex::new(crate::store::database::init(Some(format!(
            "./database_test_env_controller_{}.sqlite",
            uid
        )))));
    ...
    fn add_alarm(
        ...
        ecs.on_alarm_created(id, property, msm, store, None, false)
            .unwrap();
    }
