...
use crate::{
    ...
    modulestate::interface::{ModuleMsg, ModuleStateCmd, ModuleValueParsable},
    protos::{
        env_controller::MActor,
        message::ActionResponse,
        module::{Actor, ModuleActorOwnershipRequest, RelayOutletConfig},
    },
};
...
impl ModuleCommandSender {
    ...
    pub fn send_reserve_actor(
        ...
        for actor in mactors {
            ...
            maps.get_mut(actor.get_id())
                .unwrap()
                .insert(actor.property, state);
        }
        let items: Vec<Receiver<ActionResponse>> = maps
            .into_iter()
            .map(|(k, v)| {
                let mut request = ModuleActorOwnershipRequest::new();
                request.property = v;
                self.send_cmd("oconfig", k.as_str(), Box::new(request), actor.clone())
                    .unwrap()
            })
            .collect();
