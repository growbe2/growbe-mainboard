...
mod tests {
    ...
    use super::*;
    use crate::{protos::board::HelloWord, store::database::nbr_entry, mainboardstate::hello_world};
    use std::sync::{Arc, Mutex};
    ...
    fn store_module_config_existing() {
        ...
        store
            .store_module_config(&module_id(), &config)
            .unwrap();
    ...
    fn store_module_config_wrong_type() {
        ...
        for module in get_modules() {
            ...
            store
                .store_module_config(&module, &hello_world)
                .unwrap();
    ...
    fn store_all_supported_type_are_working() {
        ...
        for module in get_modules() {
            ...
            store
                .store_module_config(&module, &config)
                .unwrap();
